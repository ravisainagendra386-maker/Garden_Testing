...
app.post("/api/consolidate-and-run", async (req, res) => {
  ...
  for (const src of sources) {
    ...
    // For consolidation: amount is the shortfall needed, apply 1% safety buffer
    // This ensures consolidation sends: shortfall + 1% safety margin
    let useAmount = Math.floor(amount * 1.01);
    useAmount = Math.max(minAmt, useAmount);
